...
if __name__=="__main__":
    ...
    with open(DATAFILE_PATH) as fin:
        for i, line in enumerate(fin.readlines()):
            ...
            try:
                data_dict[network_name][sampler][bs][dim][0] = exec_time # dl_time + cu_time + (TRACE_DATA_DICT[network_name][dim][bs][0] + TRACE_DATA_DICT[network_name][dim][bs][1]) * bs 
        ...
        with open(DATAFILE_PATH) as fin:
            for i, line in enumerate(fin.readlines()):
                ...
                exec_time = float(tokens[8])
                try:
                    data_dict[network_name][sampler][bs][dim][1] = data_dict[network_name]["default"][bs][dim][0]/exec_time # (dl_time + cu_time + (TRACE_DATA_DICT[network_name][dim][bs][0] + TRACE_DATA_DICT[network_name][dim][bs][1]) * bs)
                except Exception as e:
                    pass
    ...
    ax.set_yticks(np.arange(0.6, 2, 0.2), minor=False)
    ax.set_yticks(np.arange(0.6, 2, 0.1), minor=True)
    ax.set_ylim([0.6, 1.5])
    ...
    fig.savefig("fig_exectime_vs_datasize.png", dpi=600, bbox_inches="tight")
    fig.savefig("fig_exectime_vs_datasize.pdf", format="pdf", bbox_inches="tight")
