...
if __name__=="__main__":
    ...
    with open(DATAFILE_PATH) as fin:
        for i, line in enumerate(fin.readlines()):
            ...
            try:
                data_dict[network_name][sampler][bs][dim][0] = dl_time + cu_time + (TRACE_DATA_DICT[network_name][dim][bs][0] + TRACE_DATA_DICT[network_name][dim][bs][1]) * bs 
        ...
        with open(DATAFILE_PATH) as fin:
            for i, line in enumerate(fin.readlines()):
                ...
                exec_time = float(tokens[8])
                try:
                    data_dict[network_name][sampler][bs][dim][1] = data_dict[network_name]["default"][bs][dim][0]/(dl_time + cu_time + (TRACE_DATA_DICT[network_name][dim][bs][0] + TRACE_DATA_DICT[network_name][dim][bs][1]) * bs)
                except Exception as e:
                    pass
