...
def normalize_by_default(group):
    ...
    #     return group
    default_time = group[group['Sampler'] == 'default']['exec time'].values[0]
    # Calculate normalized speed up 'exec time'
    group['normed'] = 1 / \
        ((group['dataload time'] + group['exec time']) / default_time)
    return group


df['exec time'] = pd.to_numeric(df['exec time'], errors='coerce')
...
def gen_plot_4_fixed_image_size():
    ...
    for batch_size, sub_group in df_chosen.groupby('Batch Size'):
        ...
        plt.ylabel('Normalized Exec Time Speed Up')
        plt.legend(loc='upper right', ncol=2)
        plt.xticks(rotation=0)
        plt.ylim([0, 1.75])
        plt.grid(axis='y')
        plt.tight_layout()
        plt.savefig(f'figures/exectime_bs{batch_size}_is{image_size}.png')
    return
