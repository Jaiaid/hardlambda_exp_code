...
def normalize_by_default(group):
    ...
    #     return group
    default_time = group[group['Sampler'] == 'default']['data process time'].values[0]
    # Calculate normalized speed up 'exec time'
    group['normed'] = 1 / \
        (group['data process time'] / default_time)
    return group
...
def gen_plot_4_fixed_image_size():
    ...
    for batch_size, sub_group in df_chosen.groupby('Batch Size'):
        ...
        plt.grid(axis='y')
        plt.tight_layout()
        plt.savefig(f'figures/processtime_bs{batch_size}_is{image_size}.png')
    return
