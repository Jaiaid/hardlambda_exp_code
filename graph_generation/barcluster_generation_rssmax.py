...
# sort
df['max rss(MiB)'] = pd.to_numeric(df['max rss(MiB)'], errors='coerce')
...
def normalize_by_default(group):
    ...
    if (len(group[group['Sampler'] == 'default']['max rss(MiB)'].values)) == 0:
        return group
    default_time = group[group['Sampler'] ==
                         'default']['max rss(MiB)'].values[0]
    # Calculate normalized speed up 'dataload time'
    group['Normalized RSS'] = 1 / \
        (group['max rss(MiB)'] / default_time)
    return group
...
for (batch_size, image_size), bs_is_group in normalized_filtered_df.groupby(['Batch Size', 'Image Size']):
    ...
    pivot = bs_is_group.pivot_table(
        index='Network Arch', columns='Sampler', values='Normalized RSS', aggfunc='first')
    ...
    filename = f'bs_{batch_size}_imgdim_{image_size}_rssmax.png'
    file_path = os.path.join(directory, filename)
