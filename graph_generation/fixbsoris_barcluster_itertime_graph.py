...
def normalize_by_default(group):
    ...
    #     return group
    default_time = max(group[group['Sampler'] == 'default']['data process time'].values[0], group[group['Sampler'] == 'default']['cache update time'].values[0]) + group[group['Sampler'] == 'default']['dataload time'].values[0] 
    # Calculate normalized speed up 'exec time'
    # print(group[['data process time', 'cache update time']].max(axis=1))
    group['normed'] = 1 / \
        ((group[['data process time', 'cache update time']].max(axis=1) + group['dataload time'])/ default_time)
    return group


df['data process time'] = pd.to_numeric(df['data process time'], errors='coerce')
...
def gen_plot_4_fixed_image_size():
    ...
    for batch_size, sub_group in df_chosen.groupby('Batch Size'):
        ...
        plt.ylabel('Normalized Process Time Speed Up')
        plt.legend(loc='upper right', ncol=2)
        plt.xticks(rotation=0)
        plt.ylim([0, 1.75])
        plt.grid(axis='y')
        plt.tight_layout()
        plt.savefig(f'figures/itertime_bs{batch_size}_is{image_size}.png')
    return
