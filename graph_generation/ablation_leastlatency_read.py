...
hatchlist = ['o', 'x']
DISABLED_PATH = "ablation_data/benchmark_iteration_step_leastlatency_disabled.tsv"
ENABLED_PATH = "ablation_data/benchmark_iteration_all_enabled_step.tsv"
...
if __name__=="__main__":
    ...
    with open(DISABLED_PATH) as fin:
        for i, line in enumerate(fin.readlines()):
            ...
            try:
                disableddata_dict[network_name][bs][dim]  = dl_time
    ...
    with open(ENABLED_PATH) as fin:
        for i, line in enumerate(fin.readlines()):
            ...
            try:
                enableddata_dict[network_name][bs][dim]  = dl_time
            except Exception as e:
                pass
    ...
    ax.bar(np.arange(0, len(NETWORK_NAMES_LABEL_DICT)) + bias, datalist,
            label="Least Latency Disabled", width=width, hatch=hatchlist[1])
    ...
    # ax.set_yticks(np.arange(0, 1.5, 0.5), minor=False)
    ...
    fig.savefig("fig_ablation_leastlatency.png", dpi=600, bbox_inches="tight")
    fig.savefig("fig_ablation_leastlatency.pdf", format="pdf", bbox_inches="tight")
