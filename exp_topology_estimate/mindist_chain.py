...
if __name__=="__main__":
    ...
    chain = []
    minedge_dist = max([max(row) for row in cache_nodes_distmatrix])
    for i in range(len(cache_nodes_distmatrix)):
        for j in range(len(cache_nodes_distmatrix)):
            if i!=j and cache_nodes_distmatrix[i][j] < minedge_dist:
                chain = [i,j]
                minedge_dist = cache_nodes_distmatrix[i][j]
    ...
    while len(rem_node_list) > 0:
        ...
        min_inc = cache_nodes_distmatrix[chain[0]][rem_node_list[0]] + cache_nodes_distmatrix[chain[-1]][rem_node_list[0]]
        added_node_idx = 0
        final_inserted_pos = 0
        for node_idx in rem_node_list:
            # increment calc
            for pos in range(len(chain)):
                increment = cache_nodes_distmatrix[chain[pos]][node_idx] + cache_nodes_distmatrix[chain[pos-1]][node_idx]
